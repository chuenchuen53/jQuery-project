...
<tr>
    ...
    <td class="poem">求乞持杖子，西方鼓笛聲，坤兌應更奇，提籠有婦人。乘車駕馬，攜酒相看，萬事皆吉，必遇平安。</td>
    <td class="lucky">吉</td>
</tr>
`;
...
function updateTable(day) {
    ...
    $("#qimen-travel-formula-table tbody")
        .children("tr")
        .each(function (index) {
            $(this)
                ...
                .text(tableData[index].score + "數");
            $(this)
                .children("td:nth-child(1)")
                .children()
                .children("div:nth-child(2)")
                .text(tableData[index].時干)
                .attr("class", "custom-color-" + BaziUtil.color(tableData[index].時干));
            $(this)
                .children("td:nth-child(1)")
                .children()
                .children("div:nth-child(3)")
                .text(tableData[index].時支)
                .attr("class", "custom-color-" + BaziUtil.color(tableData[index].時支));
            $(this).children("td:nth-child(1)").children().children("div:nth-child(4)").text(tableData[index].hour);

            console.log($(this).children("td:nth-child(1)").children().children(".時干").html());

            $(this).children("td:nth-child(2)").text(tableData[index].poem);
            $(this).children("td:nth-child(3)").text(tableData[index].lucky);
            $(this)
                .children("td:nth-child(3)")
                .attr("class", tableData[index].lucky.includes("吉") ? "lucky" : "unlucky");
        });
}
...
$(document).ready(function () {
    ...
    const today = moment();
    $("#date-picker").val(today.format("YYYY-MM-DD"));
    updateTable(today);
